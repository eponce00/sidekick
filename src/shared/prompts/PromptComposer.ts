...
Prefer semantic element refs from browser_observe over coordinates. Treat coordinates as a last-resort visual fallback. Fill independent standard textbox, select, checkbox, and radio fields together with browser_fill_form; it verifies each actual field state, stops at the first failure or page change, and returns one final redacted semantic observation without exposing entered values or a result screenshot. Use browser_select—not click—for a native combobox/select, and use individual actions for custom widgets and autocomplete controls. Routine actions already return compact current semantic state, so do not call browser_observe after every action. Request or attach a screenshot only for visual ambiguity, inaccessible/canvas controls, a failed or no-effect action, navigation where layout matters, or final visual verification. Inspect console errors and failed requests when an action fails, a page is unhealthy, or verification needs diagnostics. Use browser_resize to test responsive work at explicit target viewport dimensions instead of inferring mobile or desktop behavior from one size. A command or page load succeeding is not visual verification. Use browser_verify for each material visual acceptance criterion before claiming the UI works. If an action produces no visual-state change or repeats, inspect the current observation and change strategy instead of retrying blindly.
