...
A later completed run exposed a separate recovery-accounting defect. Three
`browser_fill_form` calls verified 2/3, 1/2, and 4/5 requested fields, with
successful targeted browser actions between them. All three results were still
classified as failures, the run-wide same-tool counter incorrectly treated them
as consecutive failures, and the renderer exposed the internal guard warning as
a user-facing banner. The browser transaction now continues through independent
fields after a field-local failure, partial completion is explicit, material
field progress and successful alternative actions reset failure streaks, and
tool-guard hints remain model-facing/durable without appearing in chat.
...
5. **Verified form batching.** `browser_fill_form` fills up to 25 standard textboxes, native selects, checkboxes, and radio buttons in order. It disallows coordinates, inspects actual control state after every field, continues through independent field-local failures, stops early only when the page changes, and emits one final semantic observation without a screenshot. Partial results identify verified, failed, and skipped counts so the model retries only failed fields; the renderer presents them as an amber partial completion rather than a false all-or-nothing failure. Input text, option values, and checked choices are redacted from durable tool arguments and returned observations. See the [tool schema](../../src/shared/agentToolCatalog.ts), [runtime redaction](../../src/main/services/agentToolRuntime.ts), and [native form transaction](../../src/main/services/nativeBrowserSessionService.ts).
...
- **Recovery and safety.** Loop detection still centers on exact action fingerprints and screenshot change. It should use semantic state/effect fingerprints, strengthen destination/private-network controls, and replace inspection-only JavaScript regex filtering with a stronger side-effect boundary.
